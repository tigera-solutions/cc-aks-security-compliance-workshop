...
6. Verify the transparent mode by running the following command in one node
   ...
   ```bash
   VMSSGROUP=$(az vmss list --output table | grep -i $RESOURCE_GROUP_$CLUSTERNAME | awk -F ' ' '{print $2}')
   VMSSNAME=$(az vmss list --output table | grep -i $RESOURCE_GROUP_$CLUSTERNAME  | awk -F ' ' '{print $1}')
